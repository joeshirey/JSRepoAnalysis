...
class TestCodeProcessorHappyPath(unittest.TestCase):
    ...
    @patch('tools.code_processor.CodeProcessor._is_already_processed', return_value=False)
    @patch('tools.code_processor.CodeProcessor._get_git_info')
    @patch('tools.code_processor.get_strategy')
    def test_process_file_success(self, mock_get_strategy, mock_get_git_info, mock_is_already_processed, mock_analyze_file, mock_save_result, mock_build_bigquery_row):
        # Arrange
        file_path = "test.py"
        ...
        mock_analyze_file.assert_called_once_with(file_path, mock_strategy, mock_git_info)
        mock_build_bigquery_row.assert_called_once_with(mock_analysis_result, mock_strategy.language, file_path)
