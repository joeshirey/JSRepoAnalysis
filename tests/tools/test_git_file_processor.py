...
class TestGitFileProcessorIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        subprocess.check_call(['git', '-C', self.repo_dir, 'config', 'user.name', 'Test User'])
        subprocess.check_call(['git', '-C', self.repo_dir, 'config', 'user.email', 'test@example.com'])
        subprocess.check_call(['git', '-C', self.repo_dir, 'remote', 'add', 'origin', 'https://github.com/test_owner/test_repo.git'])
        ...
        subprocess.check_call(['git', '-C', self.repo_dir, 'commit', '-m', 'Initial commit'])
    ...
    def test_execute_in_real_repo(self):
        ...
        self.assertTrue(result['github_link'].endswith(f"test_owner/test_repo/blob/{result['branch_name']}/test_file.py"))
        self.assertEqual(len(result['commit_history']), 1)
        self.assertEqual(result['commit_history'][0]['message'].strip(), 'Initial commit')
        self.assertEqual(result['commit_history'][0]['author_name'], 'Test User')
        self.assertEqual(result['commit_history'][0]['author_email'], 'test@example.com')
